...
def get_run_name(model_name, seed):
    return f"cifar100_{model_name}_seed{seed}"
...
def profile_model_efficiency(model, current_device, input_res=(3, 32, 32), batch_sizes=(1, 64), num_warmup=5, num_iters=20):
    results = []
...
def run_experiment(use_svda_flag, current_seed=None):
    ...
    model_summary = summary(
        model,
        input_size=(1, 3, current_params["image_size"], current_params["image_size"]),
        verbose=0,
    )
    ...
    efficiency_summary = (
        profile_model_efficiency(
            model,
            device,
            input_res=(3, current_params["image_size"], current_params["image_size"]),
...
def print_training_step_stats(model, input_res=(3, 32, 32), device=None):
    if device is None:
        device = get_device()
